...
handling.
This is a fork of the `<ingestion script module
https://github.com/UoA-eResearch/mytardis_ingestion/blob/master/
src/blueprints/custom_data_types.py
...
class Username(str):
    ...
    @classmethod
    def __get_validators__(cls) -> Generator[Callable[[str], "Username"], None, None]:
    ...
    @classmethod
    def validate(cls, value: str) -> "Username":
...
class URI(str):
    ...
    @classmethod
    def __get_validators__(cls) -> Generator[Callable[[str], "URI"], None, None]:
        """One or more validators may be yieled which will be called in order to validate the
        input. Each validator will receive as an input the value returned from the previous
        validator. (As per the Pydantic help manual).
        """
    ...
    def validate(cls, value: str) -> "URI":
        ...
        object_type_match = uri_regex.search(value.lower())
...
class ISODateTime(str):
    ...
    def __get_validators__(
        cls,
    ) -> Generator[Callable[[str], "ISODateTime"], None, None]:
        """One or more validators may be yieled which will be called in order to validate the
        input. Each validator will receive as an input the value returned from the previous
        validator. (As per the Pydantic help manual).
        """
    ...
    @classmethod
    def validate(cls, value: str) -> "ISODateTime":
        """Custom validator to ensure that the value is a string object and that it matches
...
class BaseObjectType(str):
    ...
    def __get_validators__(
        cls,
    ) -> Generator[Callable[[str], "BaseObjectType"], None, None]:
        """One or more validators may be yieled which will be called in order to validate the
        input. Each validator will receive as an input the value returned from the previous
        validator. (As per the Pydantic help manual).
        """
    ...
    @classmethod
    def validate(cls, value: str) -> "BaseObjectType":
