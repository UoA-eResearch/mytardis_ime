...
class MetadataExtractor:
    ...
    def xml_to_dict(xml_string: str) -> dict:
        ...
        result_dict = {
            key: value for nested in data_dict.values() for key, value in nested.items()
        }
    ...
    def remove_at_symbol(raw_dict: dict) -> dict:
        ...
        def remove_at_recursive(obj):
            if isinstance(obj, dict):
                ...
                filtered_dict = {
                    key.replace("@", ""): remove_at_recursive(value)
...
def flatten_dict_keys_unique_id(
    dictionary: dict, separator: str = "|", prefix: str = "", id_key: str = "ID"
) -> dict:
    """
        ...
        if isinstance(value, dict):
            flattened_dict.update(
                flatten_dict_keys_unique_id(
                    value, separator, new_key + separator, id_key=id_key
                )
            )
            ...
            for item in value:
                if id_key in item:
                    ...
                    flattened_dict.update(
                        flatten_dict_keys_unique_id(
                            item, separator, item_key + separator, id_key=id_key
                        )
                    )
        else:
